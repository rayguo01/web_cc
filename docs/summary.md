...
**问题**：服务器重启后重复抓取 domain-trends。原因是 `skillCache.get()` 使用当前小时（如 "01"）作为 hourKey，但数据存储在 2 小时窗口起点（如 "00"），导致读取失败。

**修复**：更新 `get()` 函数，根据 skillId 类型使用正确的时间窗口：
- 轮换模式 (`:group`)：2 小时窗口
- 传统模式 (`domain-trends:`)：8 小时窗口
- 普通趋势：当前小时
